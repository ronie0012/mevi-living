...
interface DiningProductCardProps {
  product: DiningProduct;
  priority?: boolean;
  className?: string;
}

export function DiningProductCard({ product, priority = false, className }: DiningProductCardProps) {
    ...
    <div className={cn("group relative", className)}>
      <div className="relative overflow-hidden bg-white rounded-lg">
        ...
        {product.originalPrice && (
          <div className="absolute left-3 top-3 z-20">
            <span className="bg-accent text-white text-[11px] tracking-wider uppercase px-2.5 py-1 rounded-sm">
        ...
        <Link href={`/products/dining/${product.slug}`} className="block">
          <div className="relative aspect-[4/5] overflow-hidden">
            ...
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-300 group-hover:opacity-100">
              <div className="pointer-events-auto">
                <Link
                  href={`/products/dining/${product.slug}`}
      ...
      <div className="mt-4 text-center">
        <h3 className="font-body text-[15px] text-foreground">
          <Link href={`/products/dining/${product.slug}`} className="hover:text-accent transition-colors">
            {product.name}
          </Link>
        </h3>
        ...
        {(product.material || product.setSize) && (
          <div className="mt-2 text-xs text-gray-500 space-x-2">
            {product.material && <span>{product.material}</span>}
            {product.material && product.setSize && <span>•</span>}
            {product.setSize && <span>{product.setSize}</span>}
          </div>
        )}
      </div>
